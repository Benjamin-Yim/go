...
type g struct {
	...
	// 如果调用了 LockOsThread，那么这个 g 会绑定到某个 m 上. 有的时候g中断了恢复会要求使用原来的M执行
	lockedm       muintptr
	sig           uint32
	writebuf      []byte
	sigcode0      uintptr
...
type m struct {
	...
	createstack   [32]uintptr //  创建此线程的堆栈。stack that created this thread, it's used for StackRecord.Stack0, so it must align with it.
...
type _func struct {
	...
	funcID    abi.FuncID //  为某些特殊的运行时函数设置。set for certain special runtime functions
	flag      abi.FuncFlag
	_         [1]byte // pad
